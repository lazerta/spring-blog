...
public class BlogController {
    ...
    public String post(Blog blog, RedirectAttributes attributes, HttpSession session) {
        ...
        Blog current;
        if (blog.getId() == null) {
            current =  blogService.saveBlog(blog);
        } else {
            current = blogService.updateBlog(blog.getId(), blog);
        }

        if (current == null ) {
            attributes.addFlashAttribute("message", "post fail");
        } else {
            attributes.addFlashAttribute("message", "post success");
        }
